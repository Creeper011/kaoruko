...
from src.commands.discord.factories import ErrorEmbedFactory
from src.constants import DEFAULT_DOWNLOAD_FORMAT
    ...
    async def download(self, interaction: discord.Interaction, url: str, format: Choice[str] | None = DEFAULT_DOWNLOAD_FORMAT, quality: Choice[str] | None = None) -> None:
        ...
        try:
            ...
            if download_output.file_url:
                elapsed_text = f"Took {elapsed}s to complete and the" if elapsed is not None else ""
                content = f"Download Completed! \n-# Some informations here: {elapsed_text} filesize is {file_size_mb}MB\n\nLink: {download_output.file_url}"
                ...
                content = "Oh no.. the download has completed, but no file URL or bytes were provided. :c"
                await interaction.edit_original_response(content=content)
